...
S_wet_fan_cowl = fan_cowl_wetted_area(l_n, D_n, l_1, D_Hl, D_ef)
...
C_dengine = C_f_nacelle*FF_N*Q_g*S_wet_fan_cowl/S
...
S_wet_emp_vert = 2*1.05*S_empvert
...
Re = min(V_inf * mac_hor_emp*rho/dyn_visc, 44.62*((mac_hor_emp/k)**1.053) * M**1.16)

# Laminar friction coefficient
C_f_lamhoremp = 1.328/np.sqrt(Re)

# Turbulent friction coefficient
C_f_turbhoremp =  0.455/((np.log10(Re)**(2.58))*(1+0.144*M**2)**0.65)

IF_horemp = 1.045

C_fhoremp = laminar_horemp * C_f_lamhoremp + (1-laminar_horemp) * C_f_turbhoremp
